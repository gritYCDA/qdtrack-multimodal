...
class MTMDCDataset(CocoVideoDataset):
	...
	def __init__(self,
		...
		self.feature_pkl_root = 'work_dirs/clustering/config_runs'
		os.makedirs(self.feature_pkl_root, exist_ok=True)
	...
	def format_results(self, 
		...
		for i in range(num_vids):
			for metric in metrics:
				os.makedirs(
					osp.join(resfiles[metric], names[i].split('/')[0]), 
					exist_ok=True)
		...
		for i in range(num_vids):
			self.mca_format_track_results(
				cam_id=i,
				results=results['track_bboxes'][inds[i]:inds[i+1]],
				infos=self.data_infos[inds[i]:inds[i+1]],
				resfile=f'{resfile_path}/track/track_results_{i}.txt',
				eval_data_type=eval_data_type)
		return resfiles, names, tmp_dir

	def mca_format_track_results(self, 
								 cam_id, 
								 results, 
								 infos, 
								 resfile,
								 sampling_rate=23,
								 outdir='kaist_mtmdc_qdtrack_base/pickled_appearance_features', # short
								 eval_data_type='test'):
		with open(resfile, 'wt') as f:
			...
			for res, info in zip(results, infos):
				frame = info['frame_id'] * sampling_rate
				...
				dirname = osp.join(self.feature_pkl_root, outdir, eval_data_type)
	...
	def evaluate(self,
		...
		accs = []
		for name in names:
			gt_file = osp.join(self.img_prefix, 'annotations', 'val', name, 'gt.txt') # short
			# gt_file = osp.join(self.img_prefix, 'annotations', 'val', name, 'gt_short.txt') # short
