...
class Multicam_evaluation:
    ...
    def load_evaluation_files(self):
        if isinstance(self.track_results_folder, str):
            self.track_result_dataframes = load_track_results(
                cam_ids=self.cam_ids, track_results_folder=self.track_results_folder)
        else:
...
def load_track_results(track_results_folder,cam_ids):
    ...
    for cam_id in cam_ids:
        track_result_path = osp.join(track_results_folder, 
                                     "track_results_{}.txt".format(cam_id))
...
def load_ground_truth_dataframes(dataset_folder,
                                 working_dir,
                                 cam_ids):
    ...
    for cam_id in cam_ids:
        ...
        cam_coords = pandas_loader.load_csv(working_dir, 
                                            cam_coords_path)
...
def split_data(dataset_folder,
    ...
    def get_splitted_dataframes(n_split_parts, cam_dataframes):
        ...
        for cam_id, cam_dataframe in enumerate(cam_dataframes):
            for chunk_id, frame_nos_chunk in enumerate(frame_no_cams_splitted):
                try:
                    dataframe_chunk = cam_dataframe[cam_dataframe["Frame#"].isin(frame_nos_chunk)]
                except:
    ...
    if 'kaist' in dataset_folder:
        # eval only 23th frames
        assert len(ground_truth_dataframes) == len(track_results_dataframes)
        for cam_id in range(len(ground_truth_dataframes)):
            sampled_frames = list(range(0, 7361, sample_rate))
            gt = ground_truth_dataframes[cam_id]
            pred = track_results_dataframes[cam_id]
            sampled_gt = gt[gt["Frame#"].isin(sampled_frames)]
            sampled_gt = sampled_gt[sampled_gt["Person#"]!=0]
            sampled_pred = pred[pred["frame_no_cam"].isin(sampled_frames)]
            ground_truth_dataframes[cam_id] = sampled_gt
            track_results_dataframes[cam_id] = sampled_pred
