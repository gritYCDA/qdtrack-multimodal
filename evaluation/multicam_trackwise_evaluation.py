...
class Multicam_trackwise_evaluation:
    ...
    def initialize_base(self):
        ...
        for gt_cam_dataframe in self.ground_truth_dataframes:
            try:
                ...
                gt_cam_df_grouped = gt_cam_dataframe.groupby("Frame#", as_index=False).mean()
                gt_cam_frame_numbers = gt_cam_df_grouped["Frame#"].tolist()
    ...
    def calculate_distances(self, motmetrics_distance, tr_rows, gt_rows):
        ...
        if motmetrics_distance == Motmetrics_distance.norm2squared_matrix:
            ...
            try:
                ...
                gt_points_this_frame = list(zip(gt_rows["x"], gt_rows["y"]))
            ...
            try:
                ...
                gt_bboxes_this_frame = list(zip(gt_rows["x"]
                                                , gt_rows["y"]
                                                , gt_rows["x"] + gt_rows["width"]
                                                , gt_rows["y"] + gt_rows["height"]))
    ...
    def evaluate(self):
        ...
        for gt_df, tr_df in zip(self.ground_truth_dataframes, self.track_result_dataframes):
            ...
            for frame_no_cam in range(self.min_all_frame_numbers, self.max_all_frame_numbers + 1):
                ...
                try:
                    ...
                    gt_rows_this_frame = gt_df[gt_df["Frame#"] == frame_no_cam]
                ...
                if "ped_id" in gt_rows_this_frame.columns:
                    ...
                    try:
                        ...
                        gt_objects_this_frame = gt_rows_this_frame["Person#"].tolist()
        ...
        track_eval_res_df.to_csv(osp.join(self.evaluation_results_path, 'eval_results.csv'))
...
def load_ground_truth_dataframes(dataset_folder, working_dir, cam_ids):
    ...
    for cam_id in cam_ids:
        scenario_name = dataset_folder.split('/')[-1]
        cam_coords_path = osp.join(dataset_folder,
...
if __name__ == "__main__":
    ...
    '''
    result = Multicam_trackwise_evaluation(dataset_folder="data/kaist_mtmdc/annotations/test/s17"
                                             ...
                                             , evaluation_results_path="work_dirs/clustering/config_runs/kaist_mtmdc_qdtrack_base/multi_cam_trackwise_evaluation"
                                             , motmetrics_distance=Motmetrics_distance.iou_matrix).evaluate()

    # result = Multicam_trackwise_evaluation(dataset_folder="data/MTA_ext_short_coco/test"
    #                                          , track_results_folder="work_dirs/clustering/config_runs/mta_qdtrack_base/multicam_clustering_results/chunk_0/test"
    #                                          , cam_ids=[0,1,2,3,4,5]
    #                                          , working_dir="work_dirs"
    #                                          , evaluation_results_path="work_dirs/clustering/config_runs/mta_qdtrack_base/multi_cam_trackwise_evaluation"
    #                                          , motmetrics_distance=Motmetrics_distance.iou_matrix).evaluate()