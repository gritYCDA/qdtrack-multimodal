...
def get_cam_id_to_cam_id_to_points(dataset_path,
                                   working_dirs,
                                   cam_count,
    ...
    def get_bbox_of_row(row):
        try:
            ...
            return (row["x_top_left_BB"],
                    row["y_top_left_BB"], 
                    row["x_bottom_right_BB"], 
                    row["y_bottom_right_BB"])

    def add_overlap_points_frame(cam_id_to_cam_id_to_points,
                                 same_frame_and_no_person_id):
    ...
    try:
        group_counts = combined_df.groupby(
            ["Frame#", "Person#"]).size().to_frame('count').reset_index()
    except:
        group_counts = combined_df.groupby(
            ["frame_no_cam", person_identifier]).size().to_frame('count').reset_index()
    ...
    for index, coord_row in tqdm(group_counts.iterrows(), total=len(group_counts)):
        try:
            same_frame_no_person_id = combined_df[(combined_df["Frame#"] == coord_row["Frame#"])
                                                  & (combined_df["Person#"] == coord_row["Person#"])]
        except:
    ...
    if pickle_path is not None:
        with open(pickle_path,"wb") as pickle_file:
            pickle.dump(cam_id_to_cam_id_to_points,
                        pickle_file,
...
def get_overlapping_areas(dataset_path,
                          working_dirs,
                          cam_count,
                          person_identifier="ped_id",
                          pickle_path=None):
    ...
    def cam_points_to_convex_hull(cam_id_to_cam_id_to_points):
        ...
        for cam_id_outer, cam_id_to_points in cam_id_to_cam_id_to_points.items():
            for cam_id_inner, points in cam_id_to_points.items():
                ...
                result_cam_ids_to_cam_id_to_hull[
                    cam_id_outer][cam_id_inner] = points[hull.vertices,:]
    ...
    cam_id_to_cam_id_to_points = get_cam_id_to_cam_id_to_points(
        dataset_path,working_dirs,cam_count,person_identifier)
    cam_ids_to_cam_id_to_hull = cam_points_to_convex_hull(
        cam_id_to_cam_id_to_points)

    with open(pickle_path,"wb") as pickle_file:
        print("Writing cam_ids_to_cam_id_to_hull")
        print(pickle_path)
        pickle.dump(cam_ids_to_cam_id_to_hull,
                    pickle_file,
                    protocol=pickle.HIGHEST_PROTOCOL)
...
def get_combined_dataframe(dataset_path,
                           working_dirs,
                           cam_ids,
                           frame_count_per_cam=None,
                           person_identifier="ped_id"):
    print("Dataset path: {}".format(dataset_path))
    combined_dataset = pd.DataFrame()
    for cam_id in cam_ids:
        if isinstance(dataset_path,str):
            cam_path = os.path.join(dataset_path, 
                                    "cam_{}/".format(cam_id), 
        ...
        try:
            cam_coords = cam_coords[["Frame#",
                                     "Person#",
                                     "x",
                                     "y",
                                     "width",
                                     "height"]]
            cam_coords = cam_coords.astype({"Frame#": int,
                                            "Person#": int,
                                            "x": int,
                                            "y": int,
                                            "width": int,
        ...
        if frame_count_per_cam is not None:
            try:
                all_frame_nos = list(cam_coords["Frame#"])
            except:
            ...
            try:
                cam_coords = cam_coords[cam_coords["Frame#"].isin(get_frame_nos)]
            except:
...
def get_cam_homographies(dataset_path,
                         working_dirs,
                         cam_count,
                         person_identifier="ped_id",
                         pickle_path=None):
    ...
    for outer_cam_id,cam_id_to_points in cam_id_to_cam_id_to_points.items():
        for inner_cam_id,inner_cam_id_points in cam_id_to_points.items():
            ...
            if len(row_cam_id_points) < 4:
                continue
            if len(column_cam_id_points) < 4:
                continue
            homography_row_cam_to_column_cam, mask = cv2.findHomography(row_cam_id_points
                                                                        ,column_cam_id_points
                                                                        ,method=cv2.RANSAC
