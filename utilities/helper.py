...
def get_all_frame_numbers_via_videos(dataset_folder, cam_ids):
    ...
    '''
    #Any cam can be used
    cam_video_path = os.path.join(dataset_folder, 
                                  "cam_{}/".format(cam_ids[0]), 
                                  "cam_{}.mp4".format(cam_ids[0]))
...
def load_dataset_dataframes(dataset_folder, working_dir, cam_count , take_frame_nos):
    ...
    for cam_id in range(cam_count):
        coords_path = os.path.join(dataset_folder,"cam_{}".format(cam_id), "coords_fib_cam_{}.csv".format(cam_id))
