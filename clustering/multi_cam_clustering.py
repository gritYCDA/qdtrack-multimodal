...
def get_feature_pickle_folder(work_dirs, 
                              config_basename, 
                              dataset_type):
...
def get_feature_pickle_path(work_dirs, 
                            frame_no_cam, 
                            cam_id, 
                            config_basename, 
                            dataset_type):
    feature_pickle_folder = get_feature_pickle_folder(
        work_dirs, config_basename, dataset_type)
    os.makedirs(feature_pickle_folder, exist_ok=True)
    feature_pickle_filename = os.path.join(feature_pickle_folder, 
        "frameno_{}_camid_{}.pkl".format(frame_no_cam,cam_id))
...
def load_tracks_all_cams(cam_count, track_results_folder):
    ...
    for cam_id in range(cam_count):
        ...
        track_result_dataframe = track_result_dataframe.astype({"frame_no_cam": int,
                                                                ...
                                                                "xbr": int,
                                                                "ybr": int})
        track_result_dataframes.append({"cam_id": cam_id, 
            "track_results": track_result_dataframe})
    return track_result_dataframes
...
class Multi_cam_clustering:
    ...
    def get_person_id_tracks_pickle_path(self,
                                         config_basename, 
                                         dataset_type):
        person_id_tracks_pickle_folder = os.path.join(
            self.work_dirs,
            "clustering",
            "config_runs",
            config_basename,
        ...
        person_id_tracks_pickle_path = osp.join(
            person_id_tracks_pickle_folder, "person_id_tracks.pkl")
        return person_id_tracks_pickle_path

    def load_person_id_tracks(self, 
                              track_results_folder):
        if len(self.person_id_tracks) > 0:
            return
        self.track_result_dataframes = \
            load_tracks_all_cams(cam_count=self.cam_count,
        ...
        for track_results in self.track_result_dataframes:
            ...
            for person_id, track in person_id_to_track.items():
                self.person_id_tracks.append({"person_id" : person_id , 
                                              "cam_id" : cam_id , 
                                              "track" : track})

    def get_all_tracks_with_feature_mean(self,
                                         track_results_folder,
                                         dataset_type):

        get_person_id_tracks_pickle_path = \
            self.get_person_id_tracks_pickle_path(
                self.config_basename,dataset_type)
        ...
        for track_dict in tqdm(self.person_id_tracks):
            feature_mean = self.calculate_track_feature_mean(
                track=track_dict, dataset_type=dataset_type)
    ...
    def calculate_track_feature_mean(self, 
                                     track,
                                     dataset_type):
        ...
        for track_entry in track_list:
            ...
            feature_pickle_name = get_feature_pickle_path(
    ...
    def initialize_maximum_link_predict_distance(self,
                                                 track_results_folder,
                                                 dataset_type,
                                                 maximum_link_frames=500):
        ...
        velocity_stats = vc.get_velocity_stats(n_tail=1, step_width=1) # kaist-specific
    ...
    def initialize_overlapping_area_tester(self):
        ...
        cam_id_to_cam_id_to_hull = get_overlapping_areas(
                                                          # dataset_path=self.train_dataset_folder
                                                          dataset_path=self.test_dataset_folder
    ...
    def load_pairwise_distances(self,
                                dataset,
        ...
        if distance_cache_active and os.path.exists(pickle_path):
            ...
            distances_and_indices = get_distances_and_indices(
                dataset, self.calculate_track_distances)
    ...
    def cluster_tracks_via_hierarchical(self,
                                        dataset_type,
                                        dist_name_to_distance_weights,
                                        distance_cache_active,
                                        person_count=350,
    ...
    def calculate_track_distances(self,
                                  candidate_track_idx,
                                  partner_track_idx,
                                  dataset):
    ...
    def cluster_from_weights(self,
        ...
        def overwrite_best_weights_in_default_weights(best_weights,
                                                      default_weights):
            overwritten_weights = copy.deepcopy(default_weights)
            for _, best_weight_row in best_weights.iterrows():
                overwritten_weights[best_weight_row["dist_name"]] = \
                  best_weight_row["dist_weight"]
        ...
        clustering_results = self.cluster_tracks_via_hierarchical(
        ...
        single_cam_eval_summary  = eval_single_cam_multiple_cams(
            dataset_base=dataset_folder,
            track_results_folder=clustering_results["tracking_results"],
            cam_ids=list(range(self.cam_count)),
            working_dir=self.work_dirs)
...
def splitted_clustering_from_weights(test_track_results_folder,
    ...
    def get_multi_cam_chunks_evaluation_path(config_basename):
        ...
        os.makedirs(folder, exist_ok=True)
        evaluations_path = os.path.join(folder,
          "chunks_evaluations.csv")
    ...
    def combine_tracking_results(eval_results):
        ...
        for result in eval_results:
            ...
            tracking_results = tracking_results.append(result_dataframe,
                ignore_index=True)
        tracking_results.sort_values('chunk_id',
            inplace=True)
        return tracking_results
    ...
    chunk_id_to_gt_chunks, chunk_id_to_tr_chunks = \
        split_data(dataset_folder=test_dataset_folder,
...
def cluster_from_weights_task(
            test_track_results_folder, # test_chunk_tr
            train_track_results_folder, 
            work_dirs,
            test_dataset_folder, # test_chunk_gt
            train_dataset_folder,
            person_identifier,
            best_weights_path,
            default_weights,
